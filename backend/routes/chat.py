...
#     session_id = request.client.host  # or use a UUID/session token if available
#     collection_name = f"user-session-{session_id.replace('.', '-')}"  # sanitize for Qdrant
...
#     async def bot_streamer():
#         context_chunks, metadata = await to_thread.run_sync(
#             lambda: query_rag(query, top_k=3, collection_name=collection_name)
#         )
