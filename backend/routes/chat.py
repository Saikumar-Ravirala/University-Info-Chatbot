...
#     session_id: Optional[str] = Form(default=None)
# ):
#     if (not files or all(f is None or isinstance(f, str) for f in files)) and not urls:
#         return JSONResponse({"error": "Missing files or URLs"}, status_code=400)
...
#     try:
#         # ✅ Handle PDF uploads (skip empty or str values Swagger sends)
#         if files:
#             for file in files:
#                 if not file or isinstance(file, str):  # skip if Swagger sent empty field
#                     continue
#                 file_path = os.path.join(temp_dir, file.filename)
#                 with open(file_path, "wb") as f:
#                     f.write(await file.read())
...
#                     lambda: index_pdfs_to_qdrant(pdf_paths, file_names, collection_name)
#                 )
#                 logger.info(f"Indexed PDFs for session {session_id}")
...
#     except Exception as e:
#         logger.error(f"Ingestion failed for session {session_id}: {str(e)}")
#         return JSONResponse(
#             {"error": f"Ingestion failed: {str(e)}"}, status_code=500
#         )
#     finally:
#         shutil.rmtree(temp_dir, ignore_errors=True)
...
#     return JSONResponse(
#         {
#             "message": "Documents and URLs indexed successfully",
#             "status": "completed",
#             "session_id": session_id,
#         }
#     )
...
#         return {"error": "Missing URL or selectors"}

#     try:
#         data = await scrape_page(url, selectors)
